...
namespace pocketmine {
	...
	const GENISYS_API_VERSION = '3.0.0';
	...
	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run Mine102PE using the CLI.");
		++$errors;
	}
	...
	if(extension_loaded("pocketmine")){
		if(version_compare(phpversion("pocketmine"), "0.0.1") < 0){
			$logger->critical("You have the native Mine102PE extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("pocketmine"), "0.0.4") > 0){
			$logger->critical("You have the native Mine102PE extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}

	if(extension_loaded("xdebug")){
		$logger->warning("You are running Mine102PE with Xdebug enabled. This has a major impact on performance.");
	}
	...
	if(\Phar::running(true) === ""){
		$logger->warning("Non-packaged Mine102PE installation detected, do not use on production.");
	}
